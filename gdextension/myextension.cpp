...
void MyExtension::_bind_methods() {
  ClassDB::bind_method(D_METHOD("optimize_frames", "offsets", "neighbors",
                                "weights", "matrices", "fixed_frame"),
                       &MyExtension::optimize_frames);
  ClassDB::bind_method(
      D_METHOD("optimize_offsets", "offsets", "neighbors", "weights", "uvs"),
      &MyExtension::optimize_offsets);
}
...
/// @brief Since our graph does not have to contain every seed (because there
/// can be inactive seeds), we need to build a mapping from the global seed
/// indices (for ex. `[0, 1, 2, 3, ...]`) to a continous index we can use in our
/// functions here (which does not include gaps. For ex. if seed 2 is inactive:
/// `[0 -> 0, 1 -> 1, 3 -> 2, ...]`)
/// @return a map from the original index to the continous index
std::unordered_map<int, int>
create_node_mapping(const TypedArray<int> &offsets,
                    const TypedArray<int> &neighbors) {
  ...
  return node_to_unknown;
}

TypedArray<Vector3>
MyExtension::optimize_frames(TypedArray<int> offsets, TypedArray<int> neighbors,
                             TypedArray<float> weights,
                             TypedArray<Transform3D> matrices,
                             Vector3 fixed_frame) {
  std::unordered_map<int, int> node_to_unknown =
      create_node_mapping(offsets, neighbors);
  ...
  result.fill(Vector3(0.0, 0.0, 0.0));
  result[0] = fixed_frame;

  for (auto &[node_id, unknown] : node_to_unknown) {
    Vec3 t = optimal_frames.segment<3>(3 * unknown);
    Vector3 t_godot = Vector3(t.x(), t.y(), t.z());
    result[node_id] = t_godot;
  }

  return result;
}

TypedArray<Vector2> MyExtension::optimize_offsets(TypedArray<int> offsets,
                                                  TypedArray<int> neighbors,
                                                  TypedArray<float> weights,
                                                  TypedArray<Vector2> uvs) {
  std::unordered_map<int, int> node_to_unknown =
  ...
  result.resize(offsets.size() / 2);
  result.fill(Vector2(0.0, 0.0));
  // I know this is technically not needed, but just to be explicit about it:
  // Fix the first offset to be (0, 0)
  result[0] = Vector2(0.0, 0.0);

  for (auto &[node_id, unknown] : node_to_unknown) {
    Vec2 u = optimal_offsets.segment<2>(2 * unknown);
    Vector2 u_godot = Vector2(u.x(), u.y());
    result[node_id] = u_godot;
  }
